...
def test_mock_org():
    ...
    assert isinstance(mock_org, mock.MockOrganization)
    assert mock_org.master_id == mock.MASTER_ACCOUNT_ID
    assert mock_org.access_role == mock.ORG_ACCESS_ROLE
...
def test_load_org():
    mock_org = mock.MockOrganization()
    mock_org._load_org(mock.SIMPLE_ORG_SPEC)
    assert isinstance(mock_org.spec, dict)
    assert mock_org.spec == yaml.safe_load(mock.SIMPLE_ORG_SPEC)
...
def test_complex_build():
    mock_org = mock.MockOrganization()
    mock_org.complex()
    assert mock_org.spec == yaml.safe_load(mock.COMPLEX_ORG_SPEC)
